...
data_type_simple
    -> data_type_text {% x => track(x, { name: toStr(x, ' '), special: true }) %}
    | data_type_numeric  {% x => track(x, { name: toStr(x, ' '), special: true }) %}
    | data_type_date
    | qualified_name
...
data_type_date
     ->  (%word {% anyKw('timestamp', 'time') %}) (%kw_with | %word {% kw('without') %}) (%word {% kw('time') %}) (%word {% kw('zone') %})
        {% x => track(x, { name: toStr(x, ' '), special: true }) %}
     | (%word {% anyKw('timestamp', 'time') %}) (lparen int rparen {% get(1) %}) (%kw_with | %word {% kw('without') %}) (%word {% kw('time') %}) (%word {% kw('zone') %})
        {% x => track(x, { name: `timestamp ${toStr(x[2])} time zone`, special: true, config: [unbox(x[1])] }) %}
