...
function visitSeqOpts(
  m: IAstVisitor,
  cs: AlterSequenceSetOptions | CreateSequenceOptions
) {
  if (cs.as) {
    ret.push("AS ");
    m.dataType(cs.as);
...
const visitor = astVisitor<IAstFullVisitor>((m) => ({
  ...
  member: (e) => {
    ...
    ret.push(e.op);
    ret.push(
      typeof e.member === "number" ? e.member.toString(10) : literal(e.member)
    );
  },
