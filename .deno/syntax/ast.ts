...
export function locationOf(node: PGNode): NodeLocation {
  ...
  if (!n) {
    throw new Error(
      "This statement has not been parsed using location tracking (which has a small performance hit). "
    );
  }
  return n;
}
...
export interface CreateMaterializedViewStatement
  extends CreateViewStatementBase {
...
export type WithStatementBinding =
  | SelectStatement
  | WithStatement
  | WithRecursiveStatement
  | InsertStatement
